      ...
      case 'INT':
        ...
        if (data.constraints?.min !== undefined && data.value < data.constraints.min) {
          ctx.addIssue({
            code: 'custom',
            path: ['value'],
            message: `Value must be >= ${data.constraints.min}`,
          });
        }
        if (data.constraints?.max !== undefined && data.value > data.constraints.max) {
          ctx.addIssue({
            code: 'custom',
            path: ['value'],
            message: `Value must be <= ${data.constraints.max}`,
          });
        }
        break;
      case 'FLOAT':
...
export class UpsertConfigBodyDto {
  @ApiProperty({
    example: 'INT',
    enum: ['INT', 'FLOAT', 'BOOL', 'STRING', 'JSON'],
  })
  type!: string;
...
export class ConfigResponseDto {
  ...
  @ApiPropertyOptional({
    example: 'Maximum receipt retry count',
    nullable: true,
  })
