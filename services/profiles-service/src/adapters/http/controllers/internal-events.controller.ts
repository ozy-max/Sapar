...
import { consumerEventsTotal } from '../../../observability/metrics.registry';
...
export class InternalEventsController {
  ...
  constructor(
    ...
    tripCompletedHandler: OnTripCompletedHandler,
  ) {
    this.handlerMap = new Map<string, EventHandler>([
      [tripCompletedHandler.eventType, tripCompletedHandler],
    ]);
  }
  ...
  async handleEvent(
    ...
    if (existing) {
      ...
      consumerEventsTotal.inc({ eventType: envelope.eventType, result: 'duplicate' });
    ...
    consumerEventsTotal.inc({ eventType: envelope.eventType, result: 'processed' });
    this.logger.log({
      msg: 'Event consumed',
      eventId: envelope.eventId,
      eventType: envelope.eventType,
