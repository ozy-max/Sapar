...
**Concurrency:** `cancel-in-progress: true` по ref.

#### Job A: Detect Changes

Скрипт `scripts/changed-services.sh` определяет затронутые сервисы по git diff. Только они проходят CI.

#### Job B: Service CI (матрица)
...
| 3 | `bash scripts/check-migrations.sh <service>` |
| 4 | `npx prisma generate` |
| 5 | `npx eslint '{src,test}/**/*.ts'` |
| 6 | `npx tsc --noEmit` |
| 7 | `npx jest --runInBand --ci --coverage` |
| 8 | Порог покрытия: ≥60% (`scripts/check-coverage.sh`) |
| 9 | `npx prisma migrate deploy` |
| 10 | `npm run test:e2e` |
| 11 | Docker build (BuildKit + GHA cache) |
| 12 | Smoke test: `/health` endpoint |

CI-контейнеры: PostgreSQL 16 + Redis 7 (сервисы GitHub Actions).

#### Job C: Push Images (только `main`)

При наличии registry secrets — push образов с тегами `:sha` и `:latest`.
